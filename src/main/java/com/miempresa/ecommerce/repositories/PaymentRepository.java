...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
         ...
         */
        @Query("SELECT COALESCE(SUM(p.monto), 0) FROM Payment p WHERE p.venta.id = :ventaId")
        BigDecimal sumarPagosPorVenta(@Param("ventaId") Long ventaId);

        /**
         * Suma total de abonos de un crédito
         */
        @Query("SELECT COALESCE(SUM(p.monto), 0) FROM Payment p WHERE p.credito.id = :creditoId")
