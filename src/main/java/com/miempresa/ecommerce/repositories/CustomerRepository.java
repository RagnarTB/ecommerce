...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
     ...
     * IMPORTANTE: Primero buscar aquí antes de llamar a la API
     */
    ...
    @Query("SELECT c FROM Customer c WHERE " +
            ...
            "LOWER(c.razonSocial) LIKE LOWER(CONCAT('%', :texto, '%'))")
     ...
     */
    @Query("SELECT c FROM Customer c WHERE " +
            "c.numeroDocumento LIKE CONCAT('%', :busqueda, '%') OR " +
            "LOWER(c.nombres) LIKE LOWER(CONCAT('%', :busqueda, '%')) OR " +
            "LOWER(c.apellidoPaterno) LIKE LOWER(CONCAT('%', :busqueda, '%')) OR " +
