...
public interface InstallmentRepository extends JpaRepository<Installment, Long> {
        ...
        @Query("""
                        ...
                        WHERE i.fechaVencimiento < :fecha
                          AND i.montoPendiente > 0
                          AND i.credito.estado = 'ACTIVO'
        ...
        @Query("""
                        SELECT i FROM Installment i
                        WHERE i.credito.cliente.id = :clienteId
                          AND i.fechaVencimiento < :fecha
                          AND i.montoPendiente > 0
                          AND i.credito.estado = 'ACTIVO'
                        ORDER BY i.fechaVencimiento ASC
                        """)
        ...
        @Query("""
                        ...
                        WHERE i.fechaVencimiento < CURRENT_DATE
                          AND i.montoPendiente > 0
                          AND i.credito.estado = 'ACTIVO'
                        """)
         ...
         */
        @Query("""
                        SELECT COALESCE(SUM(i.montoPendiente), 0)
                        FROM Installment i
                        WHERE i.fechaVencimiento < CURRENT_DATE
                          AND i.montoPendiente > 0
                          AND i.credito.estado = 'ACTIVO'
         ...
         */
        Installment findFirstByCreditoIdAndMontoPendienteGreaterThanOrderByNumeroCuotaAsc(
                        Long creditoId,
                        BigDecimal monto);
}
