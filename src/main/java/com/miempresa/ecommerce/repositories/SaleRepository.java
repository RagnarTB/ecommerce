...
public interface SaleRepository extends JpaRepository<Sale, Long> {
         ...
         */
        List<Sale> findByEstadoOrderByFechaVentaDesc();
        ...
        @Query("SELECT s FROM Sale s WHERE " +
                        ...
                        "(:tipoPago IS NULL OR s.tipoPago = :tipoPago) AND " +
                        "(:fechaInicio IS NULL OR s.fechaVenta >= :fechaInicio) AND " +
                        "(:fechaFin IS NULL OR s.fechaVenta <= :fechaFin) " +
        ...
        @Query("SELECT COALESCE(SUM(s.total), 0) FROM Sale s WHERE " +
                        "s.estado = 'COMPLETADA' AND s.fechaVenta BETWEEN :fechaInicio AND :fechaFin")
         ...
         */
        @Query("SELECT COALESCE(MAX(CAST(SUBSTRING(s.numeroVenta, 10) AS long)), 0) + 1 " +
                        "FROM Sale s WHERE s.numeroVenta LIKE :prefijo")
         ...
         */
        @Query("SELECT s FROM Sale s WHERE " +
                        "DATE(s.fechaVenta) = CURRENT_DATE AND s.estado = 'COMPLETADA' " +
                        "ORDER BY s.fechaVenta DESC")
         ...
         */
        @Query("SELECT s FROM Sale s WHERE " +
                        "YEAR(s.fechaVenta) = YEAR(CURRENT_DATE) AND " +
                        "MONTH(s.fechaVenta) = MONTH(CURRENT_DATE) AND " +
                        "s.estado = 'COMPLETADA' " +
                        "ORDER BY s.fechaVenta DESC")
         ...
         */
        @Query("SELECT DATE(s.fechaVenta) as fecha, COUNT(s) as cantidad, SUM(s.total) as total " +
                        "FROM Sale s WHERE s.fechaVenta BETWEEN :fechaInicio AND :fechaFin " +
                        "AND s.estado = 'COMPLETADA' " +
                        "GROUP BY DATE(s.fechaVenta) " +
                        "ORDER BY fecha DESC")
