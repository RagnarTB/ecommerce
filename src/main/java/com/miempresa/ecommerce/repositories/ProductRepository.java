...
public interface ProductRepository extends JpaRepository<Product, Long> {
        ...
        /**
         * Busca productos con stock bajo mínimo
        ...
        @Query("SELECT p FROM Product p WHERE " +
                        "(p.precioOferta IS NOT NULL AND p.precioOferta BETWEEN :precioMin AND :precioMax) OR " +
         ...
         */
        @Query("SELECT p FROM Product p WHERE p.precioOferta IS NOT NULL AND " +
                        "p.precioOferta > 0 AND p.precioOferta < p.precioBase AND p.activo = true")
        ...
        @Query("SELECT p FROM Product p WHERE " +
                        ...
                        "  (p.precioOferta IS NULL AND p.precioBase >= :precioMin)) AND " +
                        "(:precioMax IS NULL OR " +
                        "  (p.precioOferta IS NOT NULL AND p.precioOferta <= :precioMax) OR " +
                        "  (p.precioOferta IS NULL AND p.precioBase <= :precioMax)) AND " +
                        "p.activo = true")
